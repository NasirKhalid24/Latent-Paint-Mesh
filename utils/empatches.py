...
class EMPatches(object):
    ...
    def merge_patches(self, img_patches, indices, mode='overwrite'):
        ...
        if mode == 'min':
            if rgb:
                empty_image = np.zeros((orig_h, orig_w, img_patches[-1].shape[-1])).astype(np.float32) + np.inf # using float here is better
            else:
                empty_image = np.zeros((orig_h, orig_w)).astype(np.float32) + np.inf # using float here is better
        else:
            if rgb:
                empty_image = np.zeros((orig_h, orig_w, img_patches[-1].shape[-1])).astype(np.float32)# using float here is better
            else:
